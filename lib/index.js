...
//#region src/index.ts
/** Cordis plugin name. */
const name = "mobile-shell";
/** Services the host half needs. */
const inject = ["webServer"];
/** Runtime schema for {@link Config}. */
const Config = Schema.object({
	narrowMaxWidth: Schema.number().default(1023),
	documentTitle: Schema.string().default("DSH · {host}")
});
/** Publish the settings the browser half needs. */
function apply(ctx, config) {
	if (!Number.isFinite(config.narrowMaxWidth) || config.narrowMaxWidth <= 0) throw new Error(`mobile-shell: narrowMaxWidth must be a positive number, got ${String(config.narrowMaxWidth)}`);
	ctx.effect(() => ctx.webServer.register({
		...
		handler: (_req, res) => {
			...
			res.end(JSON.stringify({
				narrowMaxWidth: config.narrowMaxWidth,
